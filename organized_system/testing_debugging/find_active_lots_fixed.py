#!/usr/bin/env python3
"""
Find active lots and test NextJS integration - FIXED VERSION
"""
        ...
        if response.status_code == 200:
            ...
            if results and 'hits' in results[0]:
                ...
                for i, hit in enumerate(hits[:5], 1):  # Test first 5 lots
                    ...
                    try:
                        lot_response = nextjs_system.session.get(lot_page_url, timeout=10)
                        
                        if lot_response.status_code == 200:
                            html_content = lot_response.text
                            ...
                            print(f"   ❌ HTTP {lot_response.status_code}")
                ...
                print(f"❌ No lots found in Typesense results")
                if results:
